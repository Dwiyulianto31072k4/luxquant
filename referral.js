...
class ReferralSystem {
  constructor() {
    ...
    this.validKolCodes = [
      ...
      'SM138', 'MRCOWHALE', 'BHYUKNGLO', 'FARIDWHALEBTC', 'ERIKDI', 'thomatomb', 'tss07', 'brndn', 'febrian', 'crossteam', 'Neilsen', 'CYBRFI', 'ejeha', 'ndratraderalgo'
    ];
  ...
  validateInputRealtime() {
    ...
    if (value && this.validKolCodes.includes(value)) {
  ...
  verifyReferralCode() {
    ...
    if (this.validKolCodes.includes(enteredCode)) {
      ...
      this.showStatus(`🎉 Valid KOL code! ${this.discountPercent}% discount applied successfully.`, 'success');
  ...
  showDiscountPrice() {
    this.elements.priceAmount.textContent = `$${this.discountPrice.toFixed(2)}`;
    this.elements.originalPriceSpan.style.display = 'none';
    
    this.elements.discountInfo.innerHTML = `
      <div class="original-price">Regular: <span style="text-decoration: line-through;">$${this.originalPrice.toFixed(2)}</span></div>
      <div class="discount-price">KOL Discount: <span style="color: #4ade80; font-weight: 700;">$${this.discountPrice.toFixed(2)} (${this.discountPercent}% OFF)</span></div>
    `;
    this.elements.discountInfo.style.display = 'block';
  }

  hideDiscountPrice() {
    this.elements.priceAmount.textContent = `$${this.originalPrice.toFixed(2)}`;
  ...
  updateButtonText() {
    if (this.elements.btnText) {
      this.elements.btnText.textContent = `Get Discounted Access ($${this.discountPrice.toFixed(2)})`;
    }
  }
  ...
  async copyToClipboard() {
    try {
      await navigator.clipboard.writeText(this.generatedAccessCode);
  ...
  handleAccess() {
    const telegramAdmin = 'luxquantadmin';
